...
namespace Debugger__CRC_
{
    public partial class frmCRC : Form
    {
        ...
        // Méthode qui calcule le CRC d'une donnée.
        {
            ...
            CRCVal = 0; //(BUG 1) IL FAUT AJOUTER CA 
            ...
            for (int i = 0; i < Donnée.Length; i++)
            {
                ...
                //(BUG 2) IL FAUT METTRE >='0' ET <='9' SINON LE PROGRAMME NE PREND PAS EN COMPTE LES 0 ET LES 9
                {
                    NbChiffres++;
                    CRCVal = CRCVal + ((int)c - (int)'0');
                    if (CRCVal >= 100) // On ne peut pas dépasser 100 parce qu'on n'a que deux chiffres pour le CRC
                    //(BUG 3) MEME CHOSE QUE BUG 2, IL FAUT METTRE >= SINON LE SINON NUMERO 100 N'EST PAS PRIS EN COMPTE DANS LES NUMEROS A PLUS DE 2 CHIFFRES
                        CRCVal = CRCVal - 100;
                }
            }
